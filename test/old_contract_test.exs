defmodule OldOldContractTest do
  use ExUnit.Case
  doctest OldContract
  ...
  test "cast/2" do
    ...
    assert {:ok, _} =
             params |> OldContract.cast(%{foo: :string, bar: :integer, test: {:array, :integer}})

    assert {:error, _} = params |> OldContract.cast(%{bar: :string})
  end

  test "cast/2 removes unknown parameters" do
    params = %{"foo" => "Bar", "bar" => "baz", "test" => 1}

    assert {:ok, casted_params} = params |> OldContract.cast(%{foo: :string})
  ...
  test "cast/2 with atoms" do
    ...
    assert {:ok, %{foo: _, test: _}} =
             params |> OldContract.cast(%{foo: :string, test: {:array, :integer}})

    assert {:error, _} = params |> OldContract.cast(%{test: :string})
  end
  ...
  test "cast/2 with some fields null" do
    ...
    params |> OldContract.cast(%{foo: :string, test: :string, bar: :integer})
  end
  ...
  test "validate/2 confirmation" do
    ...
    assert {:ok, _} = params |> OldContract.validate(%{password: :confirmation})

    assert {:error, _} =
             %{params | password_confirmation: nil}
             |> OldContract.validate(%{password: :confirmation})
  ...
  test "validate/2 with atom keys" do
    ...
    assert {:ok, _} = params |> OldContract.validate(%{foo: :required})
    assert {:error, _} = params |> OldContract.validate(%{bar: :required})
